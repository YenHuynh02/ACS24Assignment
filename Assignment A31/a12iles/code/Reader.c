...
BufferPointer readerCreate(corex_intg size, corex_intg increment, corex_intg mode) {
    ...
    readerPointer->size = size != 0 ? size : READER_DEFAULT_SIZE;
    readerPointer->increment = increment != 0 ? increment : READER_DEFAULT_INCREMENT;
...
BufferPointer readerAddChar(BufferPointer const readerPointer, corex_char ch) {
    ...
    if (readerPointer->position.wrte * sizeof(corex_char) < readerPointer->size) {
    ...
    readerPointer->histogram[ch]++;  // Ensure ch is within bounds
