...
/* External objects */
extern corex_intg syntaxErrorNumber; /* number of syntax errors reported by the parser */
extern corex_intg line; /* source code line number - defined in scanner.c */
...
extern corex_void startParser(corex_void);
extern corex_intg startScanner(BufferPointer sc_buf);

static corex_void printParserError(corex_string fmt, ...);
static corex_void displayParser(BufferPointer ptrBuffer);
static corex_long getParserFileSize(corex_string fname);
static corex_void callGarbageCollector(corex_void);
...
corex_intg mainParser(corex_intg argc, corex_string* argv) {
	...
	if (contentBuffer == NULL) {
		printParserError("%s%s%s", argv[0], ": ", "Could not create content buffer");
		exit(EXIT_FAILURE);
	}
...
corex_void printParserError(corex_string fmt, ...) {

	va_list ap;
	va_start(ap, fmt);

	(corex_void)vfprintf(stderr, fmt, ap);
	va_end(ap);
...
corex_long getParserFileSize(corex_string fname) {
	FILE* input;
	corex_long flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
corex_void displayParser(BufferPointer ptrBuffer) {
