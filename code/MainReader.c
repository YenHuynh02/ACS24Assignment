 ...
 * -------------------------------------------------------------
 */
corex_void bErrorPrint(corex_string fmt, ...);
corex_void displayBuffer(Buffer* ptr_Buffer);
corex_long getFileSize(corex_string fname);
corex_intg isNumber(const corex_string ns);
corex_void startReader(corex_string, corex_string, corex_char, corex_intg, corex_intg);
...
corex_intg mainReader(corex_intg argc, corex_string* argv) {
	...
	corex_string program = argv[0];
	corex_string input = argv[2];
	corex_char mode = MODE_FIXED;
	corex_intg size = 0, increment = 0, wrongNumber = 0;
...
************************************************************
*/
corex_void startReader(corex_string program, corex_string input, corex_char mode, corex_intg size, corex_intg increment) {

	BufferPointer bufferp;		/* pointer to Buffer structure */
	FILE* fileHandler;			/* input file handle */
	corex_intg loadSize = 0;		/* the size of the file loaded in the buffer */
	corex_char symbol;			/* symbol read from input file */
